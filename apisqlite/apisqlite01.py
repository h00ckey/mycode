...
# search for all movies containing string
def movielookup2(mykey, searchstring):
    """Interactions with OMDB API
       mykey = omdb api key
       searchstring = string to search for"""
    try:
        # begin constructing API
        api = f"{OMDBURL}apikey={mykey}&xtypes={searchstring}"
...
def main():
    ...
    while True:
        ...
        if answer in ["1", "2"]:
            ...
            mytype = input("Search all movies in the OMDB. Enter Type (movies,series,episode):")

            if answer == "1":
                resp = movielookup(mykey, searchstring)
            elif answer == "2":
                resp = movielookup(mykey, searchstring, mytype)
